    ...
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        ...
        if self.handle_action(action_or_state):
            ...
            if not self.engine.player.is_alive:
                # the player waas killed sometime during or after the action
...
class HistoryViewer(EventHandler):
    """Prints the message_log history on a larger window which can be navigates."""
    def __init__(self, engine: Engine):
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[ActionOrHandler]:
        """By default, any mouse cilck exits this input handler."""
        return self.on_exit()
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        """Render an inventory menu, which displays the items in the inventory, and the letter corresponding to it.
        Will move to a different position based on where the player is located, so the player knows where they are.
        """
        ...
        if number_of_items_in_inventory > 0:
            ...
            console.print(
                x=x+1,
                y=y+1,
                string='(Moths fly out of your satchel.)'
            )
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if 0 <= index <= 26:  # For the 26 characters in the English alphabet
            try:
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        """Chec kfor key movement or confirmation keys."""
        key = event.sym
        if key in MOVE_KEYS:
